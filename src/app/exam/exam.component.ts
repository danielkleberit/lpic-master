...
export class ExamComponent {
  ...
  constructor(private service: QuizService, private daniel: Router, private shs:SharedService) { }
    ...
    if (QuestionType === 'fill-in') {
      ...
      if (this.answer[0]===undefined){
        ...
        if (this.answer[0] === currentQuestionanswer) {
          this.correctAnswerZahl++;
          this.currentIndex++;
          console.log('this.correctAnswerZahl',this.correctAnswerZahl)
        }
        else {
          this.falseAnswerZahl++
          this.currentIndex++;
          console.log('falseAnswerZahl',this.falseAnswerZahl)
        }
      }

    }

    if (this.falseAnswerZahl > this.Questions.length*0.2 || this.falseAnswerZahl==this.Questions.length) {
